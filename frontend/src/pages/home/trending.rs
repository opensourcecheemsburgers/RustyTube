...
pub fn TrendingSection() -> impl IntoView {
    ...
    view! {        <HomepageSection>
                ...
                <Suspense fallback=move || view! {<VideoPreviewCardPlaceholderArray />}>
                    {
                        move || trending_resource.read().map(|trending_videos_res| {
                            match trending_videos_res {
                                Ok(trending) => view! {<TrendingVideos trending=trending />},
                                Err(err) => view! {<FerrisError error=err/>}
                            }
...
pub fn TrendingHeader(category: RwSignal<TrendingCategory>) -> impl IntoView {
    ...
    view! {        <div class="pl-4 flex flex-row gap-x-3">
...
pub fn TrendingVideos(trending: Trending) -> impl IntoView {
    ...
    view! {        <div class="flex flex-row flex-wrap gap-y-12 h-[calc(100vh-64px-4rem-128px)] pb-12 overflow-y-auto scroll-smooth">
