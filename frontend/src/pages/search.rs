...
use crate::utils::{VideoQuerySignal, get_current_video_query_signal};
...
pub fn SearchResults(search_results: SearchResults, search_args: SearchArgs) -> impl IntoView {
    ...
    let search_results_view = move ||
        results_vec
            ...
            .into_iter()
            .map(|result_page| 
                result_page.into_iter().map(|result| 
                    match result {
                    SearchResult::Channel(channel) => view! { <ChannelPreviewCard channel=channel/> },
                    SearchResult::Video(video) => view! { <VideoPreviewCard video=video/> },
                    SearchResult::Playlist(playlist) => view! { <PlaylistPreviewCard playlist=playlist/> }, 
                }).collect_view()
            ).collect_view();
...
fn get_search_args_from_query_map(map: ParamsMap) -> SearchArgs {
    ...
    let response_type = map.get("type").map(|response_type| ResponseType::from_str(response_type).ok()).flatten();
    ...
    let features = map.get("features").map(|features| {
        ...
        features.split(',').for_each(|feature| if let Ok(feature) = Feature::from_str(feature) {features_vec.push(feature)});
        features_vec
    });
...
async fn fetch_search_results(args: SearchResultFetchArgs) -> Result<(), RustyTubeError> {
    ...
    let search_results = SearchResults::fetch_search_results(&args.server.get(), args.search_args, page_number)
        .await
        .unwrap();
...
//     let videos_view = videos.into_iter().map(|video| view! { <VideoPreviewCard video=video/> }).collect_view();
...
//     let channels_view = channels.into_iter().map(|channel| view! { <ChannelPreviewCard channel=channel/> }).collect_view();
...
//     let playlists_view = playlists.into_iter().map(|playlist| view! { <PlaylistPreviewCard playlist=playlist/> }).collect_view();
