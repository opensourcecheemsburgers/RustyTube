use crate::contexts::{SubscriptionsCtx, ThemeCtx};
use crate::themes::*;

use gloo::file::Blob;
use gloo::storage::{LocalStorage, Storage};
use invidious::{Subscriptions, SUBS_KEY, NewpipeSubscriptions};
use leptos::*;
...
async fn get_subs_from_file(subs: RwSignal<Subscriptions>, event: Event) -> Result<(), RustyTubeError> {
	...
	subs.update(|existing_subscriptions| { 
        ...
        existing_subscriptions.channels.sort_by(|a, b| a.name.to_lowercase().cmp(&b.name.to_lowercase()));
...
pub fn ThemeCard(name: String) -> impl IntoView {
    ...
    let card_classes = move || {
        ...
        match theme_name.get_value().eq_ignore_ascii_case(&current_theme) {
            false => "lg:w-96 w-64 overflow-hidden rounded-lg border-2 border-base-content/20 hover:border-base-content/40 outline-base-content outline-2 outline-offset-2",
            true => "lg:w-96 w-64 overflow-hidden rounded-lg border-2 border-primary hover:border-primary outline-primary outline-8 outline-offset-8",
        }
    };
