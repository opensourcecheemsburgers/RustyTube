...
use crate::{contexts::{ServerCtx, SubscriptionsCtx}, components::{FerrisError, PlaylistPreviewCard, PlaceholderCardArray, VideoPreviewCard}};
...
fn Banner() -> impl IntoView {
    ...
    let image_classes = move || match img_loaded.get() {
        true => {
            "w-full object-center object-cover bg-neutral rounded-xl".to_string()
        }
    ...
    view! {
        <div class="w-full rounded-xl bg-neutral">
            <img decoding="async" on:load=move |_| img_loaded.set(true) src=url class=image_classes/>
        </div>
    }
}
...
fn SubscribeBtn() -> impl IntoView {
    ...
    let check_subscribed = move || {
        subscriptions_ctx.0.get().channels.into_iter().find(|sub| sub.id.eq_ignore_ascii_case(&channel_id.get_value())).is_some()
    };
    ...
    let add_sub = create_action(|args: &(String, String, SubscriptionsCtx)| {
        ...
        async move {
            subscriptions_ctx.add_subscription(&id, &name).await
        }
    });
    ...
    let remove_sub = create_action(|args: &(String, SubscriptionsCtx)| {
        ...
        async move {
            subscriptions_ctx.remove_subscription(&id).await
        }
    });
    ...
    let on_click = move |_| {
        match is_subscribed.get() {
            true => remove_sub.dispatch((channel_id.get_value(), subscriptions_ctx)),
            false => add_sub.dispatch((channel_name.get_value(), channel_id.get_value(), subscriptions_ctx)),
        }
    };
...
fn ContentCategoryButtons() -> impl IntoView {
    ...
    view! {
        <div class="flex flex-row gap-x-3">
            <button on:click=move |_| content_category.set(ContentCategory::Videos) class="btn btn-outline btn-sm rounded-lg font-normal normal-case">
                Videos
            </button>
            <button on:click=move |_| content_category.set(ContentCategory::Shorts) class="btn btn-outline btn-sm rounded-lg font-normal normal-case">
                Shorts
            </button>
            <button on:click=move |_| content_category.set(ContentCategory::Livestreams) class="btn btn-outline btn-sm rounded-lg font-normal normal-case">
                Livestreams
            </button>
            <button on:click=move |_| content_category.set(ContentCategory::Playlists) class="btn btn-outline btn-sm rounded-lg font-normal normal-case">
                Playlists
            </button>
        </div>
    }
...
fn Videos() -> impl IntoView {
    ...
    let videos = create_resource(
        ...
        |(server, id)| async move {
            Channel::fetch_channel_videos(&server, &id, None).await
        }
    );
...
fn VideosInner(videos: ChannelVideos) -> impl IntoView {
    ...
    let video_fetch_args = VideosFetchArgs::new(videos_vec, server, id, continuation, fetch_more_videos);

    let videos_view = move || videos_vec.get().into_iter().map(|video| view! { <VideoPreviewCard video/> }).collect_view();
...
async fn fetch_more_videos(args: VideosFetchArgs) -> Result<(), RustyTubeError> {
    let mut channel_videos = Channel::fetch_channel_videos(&args.server.get(), &args.id.get(), args.continuation.get().as_deref()).await?;
...
fn Shorts() -> impl IntoView {
    ...
    let shorts = create_resource(
        move || (server.get(), id_query.get().unwrap_or_default()),
        |(server, id)| async move {
            Channel::fetch_channel_shorts(&server, &id, None).await
        }
    );
...
fn ShortsInner(shorts: ChannelShorts) -> impl IntoView {
    ...
    let shorts_fetch_args = ShortsFetchArgs::new(shorts_vec, server, id, continuation, fetch_more_shorts);

    let shorts_view = move || shorts_vec.get().into_iter().map(|short| view! { <VideoPreviewCard video=short/> }).collect_view();
...
async fn fetch_more_shorts(args: ShortsFetchArgs) -> Result<(), RustyTubeError> {
    let mut channel_shorts = Channel::fetch_channel_shorts(&args.server.get(), &args.id.get(), args.continuation.get().as_deref()).await?;
...
fn Livestreams() -> impl IntoView {
    ...
    let livestreams = create_resource(
        move || (server.get(), id_query.get().unwrap_or_default()),
        |(server, id)| async move {
            Channel::fetch_channel_livestreams(&server, &id, None).await
        }
    );
        ...
        <Suspense fallback=move || {
            ...
            {move || {
                livestreams
                    .get()
                    .map(|videos| match videos {
                        Ok(channel_livestreams) => view! {<LivestreamsInner livestreams=channel_livestreams/>},
                        Err(err) => view! { <FerrisError error=err/> },
                    })
            }}
...
fn LivestreamsInner(livestreams: ChannelLivestreams) -> impl IntoView {
    ...
    let fetch_more_livestreams = create_action(|args: &LivestreamsFetchArgs| fetch_more_livestreams(*args));

    let livestream_fetch_args = LivestreamsFetchArgs::new(livestreams_vec, server, id, continuation, fetch_more_livestreams);

    let livestreams_view = move || livestreams_vec.get().into_iter().map(|livestream| view! { <VideoPreviewCard video=livestream/> }).collect_view();
...
async fn fetch_more_livestreams(args: LivestreamsFetchArgs) -> Result<(), RustyTubeError> {
    let mut channel_livestreams = Channel::fetch_channel_livestreams(&args.server.get(), &args.id.get(), args.continuation.get().as_deref()).await?;
...
fn PlaylistsInner(playlists: ChannelPlaylists) -> impl IntoView {
    ...
    let fetch_more_playlists = create_action(|args: &PlaylistsFetchArgs| fetch_more_playlists(*args));

    let playlists_fetch_args = PlaylistsFetchArgs::new(playlists_vec, server, id, continuation, fetch_more_playlists);

    let playlists_view = move || playlists_vec.get().into_iter().map(|playlist| view! { <PlaylistPreviewCard playlist=playlist/> }).collect_view();
...
async fn fetch_more_playlists(args: PlaylistsFetchArgs) -> Result<(), RustyTubeError> {
    let mut channel_playlists = Channel::fetch_channel_playlists(&args.server.get(), &args.id.get(), args.continuation.get().as_deref()).await?;
