...
pub fn RecommendedSection(video_resource: VideoResource) -> impl IntoView {
	let recommended_view = move || video_resource.read().map(|res| match res {
	...
	view! {		<div class="flex flex-col rounded-lg bg-base-200 p-4">
			<h1 class="font-semibold text-xl">Recommended</h1>
			<div class="flex flex-col space-y-6 pr-4 rounded-lg bg-base-200">
				<Suspense fallback=move || view! {<RecommendedSectionPlaceholder />}>
					{recommended_view}
				</Suspense>
			</div>
		</div>
...
pub fn RecommendedVideo(video: VideoShort) -> impl IntoView {
	...
	view! {		<div class="flex flex-row space-x-4">
			<img
				on:click=open_video
...
pub fn RecommendedVideoPlaceholder() -> impl IntoView {
	view! {		<div class="bg-base-200 flex w-[45vw] flex-col rounded-lg p-4">
