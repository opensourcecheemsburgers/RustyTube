...
use crate::{pages::video::{page::VideoResource, get_current_video_query_signal}, components::FerrisError};

#[component]
pub fn RecommendedSection(cx: Scope, video_resource: VideoResource) -> impl IntoView {
	let recommended_view = move || video_resource.read(cx).map(|res| match res {
		Ok(video) => video.recommended_videos.into_iter().map(|video_short| {
			view! {cx, <RecommendedVideo video=video_short />}
		}).collect_view(cx),
	...
	view! {cx,
		<div class="flex flex-col rounded-lg bg-base-200 p-4">
			...
			<div class="flex flex-col space-y-6 pr-4 rounded-lg bg-base-200">
				<Suspense fallback=move || view! {cx, <RecommendedSectionPlaceholder />}>
					{recommended_view}
				</Suspense>
			</div>
		</div>
...
#[component]
pub fn RecommendedVideo(cx: Scope, video: VideoShort) -> impl IntoView {
	let src = video.thumbnails.get(4).cloned().unwrap().url;

	let video_id = video.id;
	let video_id_setter = get_current_video_query_signal(cx).1;
...
#[component]
pub fn RecommendedVideoPlaceholder(cx: Scope) -> impl IntoView {
	view! {cx,
		<div class="bg-base-200 flex w-[45vw] flex-col rounded-lg p-4">
			<div class="flex flex-col space-y-4 rounded-lg pr-4">
				<div class="flex flex-row space-x-6">
					<div class="bg-base-content aspect-video w-[45%] animate-pulse rounded-xl object-cover object-center">
					</div>
...
#[component]
pub fn RecommendedSectionPlaceholder(cx: Scope) -> impl IntoView {
	(0..20).map(|_| view! {cx, <RecommendedVideoPlaceholder />}).collect_view(cx)
}
