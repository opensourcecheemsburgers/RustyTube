...
pub fn Comment(comment: Comment) -> impl IntoView {
    ...
    let reply_count = comment
        .replies_info
        .clone()
        .map_or(0, |replies| replies.replies);
    let reply_continuation = comment
    .replies_info
    .clone()
...
async fn fetch_replies(args: ReplyFetchArgs) {
    if let Some(token) = args.continuation.get() {
        let replies = Replies::fetch_replies(token.as_str(), args.server.get().as_str(), args.video_id.0.get().unwrap().as_str())
            .await
            .unwrap();
