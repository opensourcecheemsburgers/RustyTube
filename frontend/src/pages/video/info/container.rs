...
use crate::contexts::{SubscriptionsCtx, VIDEO_PLAYER_ID, PlayerState};
use crate::icons::{CalendarIcon, DownloadIcon, LikeIcon, PlaylistAddIcon, ShareIcon, ViewsIcon, DislikeIcon};
use crate::pages::subscriptions;
use crate::pages::video::page::VideoResource;
use crate::utils::get_current_video_query_signal;
use invidious::{Dislikes, Video, Subscription, Format, Formats};
use leptos::*;
...
pub fn ShareDropdown() -> impl IntoView {
    ...
    let toggle_timestamp = move |_| {
        let timestamp = timestamp_input.get().map_or(false, |timestamp_input: HtmlElement<html::Input>| timestamp_input.checked());
    ...
    let link_text = move || match link_type.get() {
        LinkType::RustyTube => match include_timestamp.get() {
            true => format!("https://rustytube.rs/player?id={}&time={}", video_id, current_time.get()),
