...
pub fn VideoPreviewCard(video: CommonVideo) -> impl IntoView {
    view! {
        ...
        >
            <Thumbnail video_id=video.id.clone() url=video.thumbnails.get(3).map(|thumb| thumb.url.clone())/>
            <Info video=video/>
        </div>
    }
}
    ...
    view! {
        <div class="flex flex-col w-full mt-3 space-y-3 px-2 cursor-text">
            ...
            <div class="flex flex-row flex-wrap items-center font-normal text-sm gap-1">
                <h2 on:click=go_to_channel_page class="cursor-pointer text-primary">{author}</h2>
                {verified_check}
                <p>{"•"}</p>
