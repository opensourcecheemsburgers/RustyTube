...
#[component]
pub fn VideoPreviewCardThumbnail(url: String, video_id: String) -> impl IntoView {
...
#[component]
pub fn VideoPreviewCardPlaceholderArray() -> impl IntoView {
    view! {
        <div class="flex flex-row flex-wrap gap-y-12 h-[calc(100vh-13rem)] overflow-y-scroll">
            {(0..50).map(|_| view! { <VideoPreviewCardPlaceholder/> }).collect_view()}
        </div>
    }
}

#[component]
pub fn VideoPreviewCardPlaceholder() -> impl IntoView {
    view! {
        <div class="basis-1/3 lg:basis-1/4 flex flex-col h-auto px-4 overflow-hidden">
